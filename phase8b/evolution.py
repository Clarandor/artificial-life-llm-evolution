"""
Phase 8B: Evolution with Temporal Alignment
==========================================
...
Phase 8B changes:
  1. Level 2 recursive weight reduced from 1.0 to 0.3
  2. Added temporal alignment (cross-correlate historical vs current attention)
  3. rec_attn_output = rec_ctx * 0.3 + aligned_rec_ctx * 0.5
  4. Track alignment strength as a new metric
  5. Additional metric: temporal alignment stability (how much correction is needed)
"""
...
from .agent import HIDDEN_DIM, ATT_DIM, GRID_SIZE, Agent, TemporalAlignedAttention, LSTMCell, REC_WEIGHT, ALIGN_WEIGHT
...
class Evolution:
    ...
    def __init__(self, seed=42):
        ...
        for t in range(self.N_TRIBES):
            ...
            self.tribe_templates[t] = {
                ...
                'attn': TemporalAlignedAttention(HIDDEN_DIM, ATT_DIM, rng_t),
            }
        ...
        attn_p = 2 * ATT_DIM * HIDDEN_DIM + 2 * ATT_DIM * 8  # +1 for W_align
        print(f"  TemporalAlignedAttention: Level1 (Q/K) + Level2 (rec + temporal align)")
        print(f"  Params: LSTM={lstm_p} attn={attn_p} dec={self.agents[0].dec_W.size+self.agents[0].dec_b.size} total={total_params}")
        print(f"  REC_WEIGHT={REC_WEIGHT}  ALIGN_WEIGHT={ALIGN_WEIGHT}")
        ...
        for tribe_id, members in by_tribe.items():
            ...
            for _ in range(len(tribe_a)):
                ...
                for attr, frac in [
                    ...
                    (child.attn.W_rec, 0.1), (child.attn.W_align, 0.1),  # W_align is new
    ...
    def run(self, generations=100, save_log=True):
        ...
        for gen in range(generations):
            ...
            for step in range(self.STEPS_PER_GEN):
                ...
                for agent in alive:
                    ...
                    # decide() uses prev-step weights; sets _last_attn_weights
                    action = agent.decide(nh_h, neighbor_attn_weights=nh_attn_weights)
                    
                    # Track attention loss (Level 1)
                    if nh_h:
                        _, _, aw = agent.attn.attend(agent.h, nh_h, nh_attn_weights)
                        if len(aw) > 1:
                    ...
                    # Track alignment strength (from the attention module)
                    # The alignment strength tells us how much temporal correction was applied
                    align_strengths.append(agent.attn._prev_level1_weights.max())
                ...
                for agent in alive:
                    ...
                    action = agent.decide(nh_h2, neighbor_attn_weights=nh_attn2)
            ...
            mean_align_strength = np.mean(align_strengths) if align_strengths else 0.0
            ...
            # Count recursive attention usage (non-zero W_rec activations)
            rec_usage = 0
            align_usage = 0
            if alive:
                sample_agent = alive[0]
                rec_in = sample_agent.attn.W_rec @ np.ones(8)
                align_in = sample_agent.attn.W_align @ np.ones(8)
                rec_usage = float(np.mean(np.abs(rec_in)))
                align_usage = float(np.mean(np.abs(align_in)))
            ...
            log = {
                ...
                "coordination_rate": total_large / max(total_failed + total_large, 1),
                "mean_attn_entropy": float(mean_attn_entropy),
                "mean_attn_loss": float(mean_attn_loss),
                "mean_align_strength": float(mean_align_strength),
                "mean_signals_sent": float(sig_sent.mean()) if len(sig_sent) else 0.0,
                "recursive_attn_usage": rec_usage,
                "temporal_align_usage": align_usage,
            ...
            if gen % 10 == 0:
                ...
                print(f"Gen {gen:4d} | pop={len(alive):3d} | "
                      ...
                      f"Rec={rec_usage:.3f} Aln={align_usage:.3f} | "
                      f"⏱ {elapsed:.0f}s",
                      flush=True)
        ...
        if save_log:
            os.makedirs("results/phase8b", exist_ok=True)
            with open("results/phase8b/generation_log.json", "w") as f:
