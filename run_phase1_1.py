"""
Entry point: Run Phase 1.1 — Factored Attention experiment.

Phase 1.1: Reduced QK dimension (16→4), removed V projection.
Tests whether GA can find selective attention in a smaller parameter space.
Total params ~2,182 (vs Phase 1's ~3,846).

Usage:
    python run_phase1_1.py                        # default 300 generations
    python run_phase1_1.py --generations 500
    python run_phase1_1.py --seed 0
    python run_phase1_1.py --no-plots
"""
...
from phase1_1.evolution import EvolutionEngine
from phase1_1.environment import N_TRIBES, TRIBE_SIZE, NUM_PREDATORS, NUM_PREY, NEIGHBOR_RADIUS
from phase1_1.agent import FactoredAttentionMLP, HIDDEN_DIM, ATT_DIM
...
def parse_args():
    p = argparse.ArgumentParser(description="Phase 1.1: Factored Attention")
...
def main():
    ...
    print("=" * 60)
    print("  Artificial Life × LLM Evolution — Phase 1.1")
    print("  Factored Attention (ATT_DIM=4, no V projection)")
    print("=" * 60)
    ...
    print(f"    - Context = attn @ neighbor_hidden[:, :{ATT_DIM}]")
    print(f"    - Decoder: (hidden+context={HIDDEN_DIM+ATT_DIM}) → action(6)")
    print(f"    - Params ≈ {FactoredAttentionMLP().param_count}")
    ...
    if not args.no_plots:
        try:
            from phase1_1.visualize import generate_all_plots
